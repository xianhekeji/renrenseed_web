...
$filename = "Enterprise_" . $CompanyId . ".jpg"; ///要生成的图片名字  
$filePath = $imgDir . $filename;
$url = $CFG['url'] . "files/qrcode/" . $filename;
if (file_exists($filePath)) {
    echo app_wx_iconv_result_no('getEnterpriseQRCode', true, '1001', 0, 0, 0, $url);
} else {
    ...
    $data = array("scene" => $CompanyId, "page" => "pages/companyInfo/companyInfo");
    $result = http($get_qrcode_url, $data, true);
    $jpg = $result;
    if (empty($jpg)) {
        echo app_wx_iconv_result_no('getEnterpriseQRCode', false, '1002', 0, 0, 0, $url);
        exit();
    }
    ...
    fclose($file); //关闭  
    if (!file_exists($filePath)) {
        echo app_wx_iconv_result_no('getEnterpriseQRCode', false, '1003', 0, 0, 0, $url);
        exit();
    }
    echo app_wx_iconv_result_no('getEnterpriseQRCode', true, '1004', 0, 0, 0, $url);
}
